...
weather_dic = {'Clear':0,'Weak rain':1,'Mediocre rain':2,'Heavy rain':3,'Weak snow/sleet':4,'Mediocre snow/sleet':5,'Heavy snow/sleet':6,'Mist/Fog':7}
road_dic = {'Dry':0,'Moist':1,'Wet':2,'Wet and salty':3,'Frost':4,'Snow':5,'Ice':6,'Probably moist and salty':7,'Slushy':8}
# Dictionaries of weights for weather and road conditions according to severity
road_condition_weights = {'Frost':0.2,'Ice':0.18,'Snow':0.16,'Slushy':0.14,'Wet and salty':0.1,'Wet':0.09,'Moist':0.07,'Probably moist and salty':0.05,'Dry':0.01}
weather_condition_weights = {'Mist/Fog':0.2,'Heavy snow/sleet':0.18,'Mediocre snow/sleet':0.16,'Weak snow/sleet':0.14,'Heavy rain':0.12,'Mediocre rain':0.1,'Weak rain':0.08,'Clear':0.02}

def radius_calc(origStation_point,destStation_points_list, id_destStation):
	"""
	Calculate the radius and check if an origin(weather or camera station) station is inside a 200m radius of a given destination station

	Arguemnts:
	origStation_point -- tuple of the coordinates (latitude, longitude) of origin station
	destStation_points_list -- list of tuples of the coordinates (latitude, longitude) of all destination stations
...
def nearby (origStations_map, destStations_map, id_destStation):
	...
	Return:
	nearby_200m -- list of list of the ids of all the destination stations located nearby each origin station
	"""
	destStation_points_list = list(zip(destStations_map['latitude'].tolist(), destStations_map['longitude'].tolist()))
	nearby_200m = []
	for i in range(len(origStations_map)):
		origStation_point = (origStations_map.iloc[i]['latitude'], origStations_map.iloc[i]['longitude'])
	...
	if type(x)==str:
		if x.find(' / ') != -1:
			...
			if intensity in ['Weak','Mediocre','Heavy']:
				x = x+' rain'
		if x.split(' / ')[0] in ['Wet sleet','Sleet','Ice crystals','Snow grains','Graupel','Freezing drizzle','Freezing rain']:
			x = 'snow/sleet'
	return x
...
def sensors_values(data_weather):
	...
	sensors_info = [list(map(dico.get, ['id','roadStationId','oldName','sensorValue','sensorUnit','sensorValueDescriptionEn','measuredTime'])) for dico in sensors_values]
	list_columns = ['id_sensor','roadStationId','oldName','sensorValue','sensorUnit','sensorValueDescriptionEn','measuredTime']
...
def weather_stations(data_weather_stations,data_weather, weatherdf):
	...
	coord_weatherStation_dict={"weatherStationId": ids_weatherStation, 'longitude': list(coordinates_weatherStation[0]), 'latitude': list(coordinates_weatherStation[1]), "stationSensors":sensors_weatherStation}
	weatherStations_map = pd.DataFrame(coord_weatherStation_dict)
	sensors_data = weatherdf.merge(weatherStations_map[['weatherStationId','longitude','latitude']],left_on='roadStationId',right_on='weatherStationId')
...
def camera_stations(data_camera,data_camera_stations):
	...
	Return:
	cameraStations_map -- camera stations dataframe :camera_id, location and camera_station_id, nearest_weather_stations_id (to build the map)
	...
	coord_camera_station_dict={"id_cameraStation": ids_camera_station, 'longitude': list(coordinates_camera_station[0]), 'latitude': list(coordinates_camera_station[1])}
	coord_camera_stationdf = pd.DataFrame(coord_camera_station_dict)
	cameraStations_map = cameraStations.merge(coord_camera_stationdf, on='id_cameraStation', how = 'inner')
...
def weather_road_conditions(data_weather, weatherdf):
	...
	def find_condi(df,sensor1,sensor2):
		"""
		Combine the results of two similar type of sensors related to the road and weather conditions

		Arguments:
		df -- dataframe (conditiondf) contains values of the two road condition sensors for each weather station
		...
		if sensor1 in sensors and sensor2 in sensors:
			...
			if condi1 != 'The sensor has a fault' and condi2!= 'The sensor has a fault':
				if condi1 == condi2:
					road_cond=condi1
				else:
					road_cond=condi1+' / '+condi2

			elif condi1!= 'The sensor has a fault':
				road_cond=condi1
			elif condi2!= 'The sensor has a fault':
				road_cond=condi2
			else:
				road_cond=np.nan
		elif sensor1 in sensors:
			condi1 = (df[df['oldName'] == sensor1]['sensorValueDescriptionEn']).tolist()[0]
			if condi1!= 'The sensor has a fault':
				road_cond=condi1
			else:
				road_cond=np.nan
		elif sensor2 in sensors:
			condi2 = (df[df['oldName'] == sensor2]['sensorValueDescriptionEn']).tolist()[0]
			if condi2!= 'The sensor has a fault':
				road_cond=condi2
			else:
				road_cond=np.nan
		else:
	...
	for st in roadStationIdlist:
		...
		road_condition.append(find_condi(stgrpnew, 'roadsurfaceconditions1','roadsurfaceconditions2'))
		weather_condition.append(find_condi(stgrpnew, 'precipitationtype','precipitation'))
	conditiondf = pd.DataFrame(list(zip(roadStationIdlist,road_condition,weather_condition)), columns=['roadStationId','road_condition','weather_condition'])
